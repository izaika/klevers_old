<?php
	$body_attributes = [
		...
		'class'			=> isset($no_footer) && $no_footer ? 'no-footer' : null
	];
	$blue_bg = isset($blue_bg) && $blue_bg;
?><!DOCTYPE html>
<html<?= HTML::attributes(['class' => $blue_bg ? 'no-js blue-bg' : 'no-js', 'lang' => 'no']) ?>>
<head>
	<meta charset="utf-8">
<?php
switch ($device) {
...
<?php
	...
	// echo HTML::script('js/mobile/lib/require.js', ['data-main' => URL::site('js/mobile/main'.((Kohana::$environment == Kohana::PRODUCTION) ? '-built' : '').'.'.$serial_number)]);
	echo HTML::script('js/mobile/lib/require.js', ['data-main' => URL::site('js/mobile/main-built').'.'.$serial_number]);
?>
	<script>
...
<div id="container">
<?= isset($no_grid) && $no_grid ? null : View::factory('mobile/_grid') ?>
<?= $content ?>
<?= View::factory('mobile/_busy_overlay') ?>
</div>
