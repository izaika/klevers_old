...
<?php
	...
	echo Form::open('#', ['id' => 'filters_form']);
		...
		foreach ($categories as $i => $c) {
			...
			echo Form::label('cat_'.$c->id, $c->heading, ['class' => $i ? null : 'first']);
			$all_filters[] = '.cat-'.$c->id;
		}
		echo Form::radio('category', 'cat-all', ($cat_id == 0), ['id' => 'cat_all', 'data-filter' => '.isotope-item']);
		echo Form::label('cat_all', 'ALLE PROSJEKTER');
	echo Form::close();
	echo HTML::anchor('søk', 'Søk', ['class' => 'search-link transition']);
?>
	</div>
</aside>
<a href="#" class="selected-filter"><span><?= $active_category_name ?></span><?php include DOCROOT.'img/simple-arrow-down.svg' ?></a>
<main id="projects">
	<div<?= HTML::attributes([
